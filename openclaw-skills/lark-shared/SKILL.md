...
- Use this skill when the user explicitly asks for 飞书 CLI 共享认证规则 work in Feishu/Lark.
...
- Discover the relevant `lark-cli auth` command surface with `--help` before composing requests.
...
lark-cli auth --help | sed -n '1,80p'
```
