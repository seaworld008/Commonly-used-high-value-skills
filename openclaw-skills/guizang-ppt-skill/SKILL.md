...
description: '生成电子杂志 × 电子墨水风格的横向翻页网页 PPT（单 HTML 文件），含 WebGL 流体背景、衬线标题 + 非衬线正文、章节幕封、数据大字报、图片网格等模板。当用户需要制作分享 / 演讲 / 发布会风格的网页 PPT，或提到杂志风 PPT、horizontal swipe deck、editorial magazine、e-ink presentation 时使用。'
version: "1.0.0"
author: "seaworld008"
...
This local supplement keeps the upstream Chinese-language workflow compatible
with this repository's quality lint. The canonical upstream content above
remains the source of truth for how to operate the skill.

## Workflow Reference

```text
1. Clarify audience, duration, source material, image availability, theme, and hard constraints.
2. Copy assets/template.html into the target ppt/ folder and create a sibling images/ directory.
3. Choose one of the five curated themes from references/themes.md.
4. Build slides from the layout skeletons in references/layouts.md without inventing missing classes.
5. Run the checklist in references/checklist.md before delivery.
```
