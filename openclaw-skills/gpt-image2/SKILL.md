...
```text
API key:  GPT_IMAGE2_API_KEY -> OPENAI_API_KEY -> CODEX_OPENAI_API_KEY
Base URL: GPT_IMAGE2_BASE_URL -> OPENAI_BASE_URL -> OPENAI_API_BASE_URL -> OPENAI_API_BASE -> CODEX_OPENAI_BASE_URL -> CODEX_OPENAI_API_BASE_URL -> https://api.openai.com/v1
```

This lets the skill use the same OpenAI-compatible key/base URL as the surrounding Codex process when those variables are intentionally exposed to child processes. It does not read Codex desktop internal credential stores, session tokens, browser storage, config files, or hidden app state. If the runtime does not expose an API key to the shell process, ask the user to set one locally.
...
```text
当前进程没有暴露可用于画图接口的 API key。请设置 GPT_IMAGE2_API_KEY 或 OPENAI_API_KEY；如果你使用自定义服务，也可以设置 GPT_IMAGE2_BASE_URL。
```
...
```text
当前进程没有暴露可用于画图接口的 API key。请设置 GPT_IMAGE2_API_KEY 或 OPENAI_API_KEY。
```
...
The CLI reads command-line flags first, then process environment variables, then `scripts/.env`, then defaults. `GPT_IMAGE2_*` variables are the skill-specific override layer. Standard OpenAI/Codex-compatible variables are fallback layers so the skill can use the same key/base URL as Codex when the runtime exposes them.
...
If `GPT_IMAGE2_BASE_URL` is blank, the script falls back to `OPENAI_BASE_URL`, `OPENAI_API_BASE_URL`, `OPENAI_API_BASE`, `CODEX_OPENAI_BASE_URL`, `CODEX_OPENAI_API_BASE_URL`, and then `https://api.openai.com/v1`.
