...
The upstream author reports verification with Prisma `7.6.0` showing no generated client files emitted by `migrate dev` or `db push`, even though some CLI help text still says `migrate dev` "trigger[s] generators".
...
```bash
# v7 workflow
# Destructive: disposable development database only, after explicit selection.
