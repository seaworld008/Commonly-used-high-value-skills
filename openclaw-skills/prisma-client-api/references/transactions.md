...
Use the array form for independent operations whose inputs are already known.
For a write that needs a generated ID from a preceding write, use an interactive
transaction or nested writes; do not assume a generated ID is `1`.

```typescript
const [users, postCount] = await prisma.$transaction([
