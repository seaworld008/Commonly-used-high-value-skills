...
**4. Exposing tables to the Data API:** Depending on the user's [Data API settings](https://supabase.com/dashboard/projects) (open the project, then `integrations/data_api/settings`), newly created tables may not be automatically exposed via the Data (REST) API. If this is the case, `anon` and `authenticated` roles will need to be explicitly granted access.
...
- Prefer the upstream workflow for Supabase; this section only adds local quality
  guardrails.
