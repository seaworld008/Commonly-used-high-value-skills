...
| # | Rule | Discipline |
|---|------|-----------|
| Q9 | **Independent review complements producer checks** | Producers run relevant checks and supply evidence. Add an independent verifier when the project, risk or chosen evaluation recipe requires one; a producer is not the sole judge of an independently gated deliverable. Do not forbid self-checks based on an inherited model protocol, and do not repeat successful unchanged checks merely for another report. |
...
| # | Rule | Discipline |
|---|------|-----------|
| Q12a | **Spend effort where it changes the verdict** | Preserve the named model and configured effort by default. If tuning is authorized, compare representative outcomes and use the target host's documented controls; do not assume universal defaults or cache behavior across models. Keep settings stable within a controlled comparison, report any deliberate changes separately, and use concise output requirements rather than reasoning slogans to control response length. |
