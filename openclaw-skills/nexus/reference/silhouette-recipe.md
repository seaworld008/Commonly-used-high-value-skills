...
*Range derivation* (convention: `reference/recipe-contract.md` §1 element 7): floor = one surface, ACCEPT on cycle 1 → 1+3+3+3+3+... = **14**; ceiling = a surface set, 3 cycles, wide competitor pool → 1+3+6+4+(6×3=18 with shrinking re-measures ≈ 14)+3+1 = **30**. **Resolve unapproved launch scope or budget before execution** (intentional parity with `runway`/`wish`).
