...
| 参数 | 必填 | 说明 |
|------|------|------|
| `--to <emails>` | 否 | 完整收件人列表，多项重复传 `--to`，每次一个值。支持 `Alice <alice@example.com>` 格式。省略时草稿不带收件人（之后可通过 `+draft-edit` 添加） |
| `--subject <text>` | 是 | 草稿主题 |
...
| `--cc <emails>` | 否 | 完整抄送列表，多项重复传 `--cc`，每次一个值 |
| `--bcc <emails>` | 否 | 完整密送列表，多项重复传 `--bcc`，每次一个值。与 `--event-*` 不兼容（见 `+send` 日程邀请约束） |
