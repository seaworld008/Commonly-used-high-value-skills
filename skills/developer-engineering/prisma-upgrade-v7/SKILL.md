...
## Local Migration Safety and Acceptance

This is a reviewed overlay of the pinned MIT-licensed official skill.
Record the installed CLI/client versions, database provider, module format,
connection method and deployment target before selecting upgrade steps.
The MongoDB boundary above is a route decision, not permission to change providers.
The optional `prisma-mongodb-upgrade` skill is not bundled here; use official
MongoDB guidance if it is absent. Do not apply the SQL migration to MongoDB.

Run generation, typechecking and focused database integration tests on an
isolated development database. Verify transaction rollback and pool behavior.
Review migration SQL before applying it; record backup and recovery steps for
production changes. The reset examples in removed-features.md are destructive
and apply only to disposable development databases explicitly selected for reset.
Do not infer authorization to reset or migrate production from an ORM upgrade.
Record the accepted target version in the lockfile and release notes.
