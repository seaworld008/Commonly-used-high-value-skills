...
| Eureka | `eureka` |  | **Novelty-proven invention** — prove a mechanism new against a Prior-Art Ledger. **Require authorization for the declared scope and budget; reuse existing approval** (`depth=scout` excepted). Contract: `ship=true` is opt-in only (never inferred), mutually exclusive with `depth=scout`. | See `reference/eureka-recipe.md` | `reference/eureka-recipe.md`, `reference/research-grounding.md` |
